...
| `worker` | 확장 측정용 전달 Worker | profile 사용 시에만 실행, 호스트에 공개하지 않음 |
...
스크립트는 API 애플리케이션의 Worker를 끈 뒤 `worker-scale` profile의 Worker를 1개·2개·4개로 확장합니다. 대기열 500건과 1,000건을 각 조건에서 3회 처리하고 처리량, Worker별 선점 수, 선점 쿼리 횟수와 평균 시간을 출력합니다.

이벤트·전달·시도 이력·수신 요청 수가 다르거나 Worker가 한 건도 선점하지 못하면 실행을 중단합니다. 실패 시 확장 Worker와 수신기의 마지막 100줄을 출력하고 임시 Worker를 제거한 뒤 기본 설정을 복구합니다. 측정 조건과 결과는 [다중 Worker 확장 측정](worker-scaling.md)에 기록했습니다.
