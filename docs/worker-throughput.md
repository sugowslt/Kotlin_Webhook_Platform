...
이번 결과에는 단일 애플리케이션·단일 Worker, 로컬 네트워크, 응답 지연 0ms, 성공 응답만 포함됩니다. TLS, 외부 네트워크 지연, 재시도, timeout, DB 크기별 변화는 측정하지 않았습니다. 다중 Worker 비교는 [다중 Worker 확장 측정](worker-scaling.md)에 분리했습니다. 운영 용량이나 SLO를 정하려면 실제 배포 환경과 예상 지연 분포로 다시 측정해야 합니다.
