...
- [x] 상태별 전달 목록 조회
- [x] 생성 시각과 전달 ID를 사용한 cursor 페이지
- [x] 전달 상세와 시도 이력 조회
- [x] 조회·재전송 API의 운영자 권한 통합
- [x] payload, endpoint URL, 서명 비밀값 미노출
- [x] 목록 정렬과 상태 필터용 PostgreSQL 인덱스

운영자는 실패한 전달 ID를 목록에서 찾고 상세 시도 이력을 확인한 뒤 같은 권한으로 재전송할 수 있습니다. 전체 건수 조회 없이 `limit + 1`건으로 다음 페이지를 판단하고, 생성 시각과 UUID를 묶은 keyset cursor로 페이지 사이 중복을 막았습니다.
